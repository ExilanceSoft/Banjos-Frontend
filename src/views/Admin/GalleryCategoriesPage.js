...
const GalleryCategoriesPage = () => {
  ...
  const handleAddCategory = async (e) => {
    ...
    try {
      ...
      await axios.post('http://64.227.163.17:8000/gallery_cat/categories/add', formDataToSend, {
        headers: {
  ...
  const handleEditCategory = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `http://64.227.163.17:8000/gallery_cat/categories/${selectedCategory.id}`,
        formDataToSend,
        {
          headers: {
            'Content-Type': 'multipart/form-data'
  ...
  const handleDeleteCategory = async (id) => {
    if (window.confirm('Are you sure you want to delete this category?')) {
      try {
        await axios.delete(`http://64.227.163.17:8000/gallery_cat/categories/${id}`);
        fetchCategories();
      } catch (err) {
